...
function HandyCAPSSlider(cont)  {
	...
	this.highlightBul = function(index) {
		var bullets = this.targetBullets();
		bullets[index].style.backgroundColor = this.bulAltCol;
	};

	this.resetBul = function(index) {
		var bullets = this.targetBullets();
		bullets[index].style.backgroundColor = this.bulColor;
	};

	this.hightlightMinis = function(index) {
		this.miniKids[index].style.zIndex = '2';
		this.miniKids[index].style.transform = 'scale(1.125)';
		this.miniKids[index].style.webkitTransform = 'scale(1.125)';
	};

	this.resetMinis = function(index) {
		this.miniKids[index].style.zIndex = '1';
		this.miniKids[index].style.transform = 'scale(1)';
		this.miniKids[index].style.webkitTransform = 'scale(1)';
	};
	...
	this.makeMinis = function() {
		...
		this.miniKids = miniJack.children;

		var width = 100 / this.itemCount,
		trans = this.getSeconds(this.animDur) / 2;

		for (var i = 0; i < this.miniKids.length; i++) {
			this.miniKids[i].className = 'mini' + i;
			this.miniKids[i].setAttribute('style', 'width: ' + width + '%; cursor: pointer;  transition: all 0.6s ' + trans + 's');
			this.miniKids[i].addEventListener('click', this.miniCb.bind(this));
		}
	};
	...
	this.init = function(paras) {
		...
		this.doMinis		= params.minis === undefined || params.minis === true ? true : false;
		this.blowMinis	= params.highlightMinis === undefined || params.highlightMinis === true ? true : false;
